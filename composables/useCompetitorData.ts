...
/**
 * Extracts competitor mention data from query results
 */
export function useCompetitorMentionData(queries: Ref<any[]>, competitors: Ref<any[]>) {
  // Competitor mention data
  const competitorsWithMentionData = computed(() => {
    const competitorsList = competitors.value || []
    const queriesList = queries.value || []
    ...
    if (competitorsList.length > 0) {
      return competitorsList.map(comp => {
        ...
        return {
          ...
          mentionRate: mentionData ? (mentionData.mentions / queriesList.length) * 100 : 0,
    ...
    return Array.from(competitorMentions.values())
      .map(data => ({
        ...
        mentionRate: (data.mentions / queriesList.length) * 100,
  ...
  return {
    competitorsWithMentionData
  }
}

/**
 * Extracts competitor citation data from page analyses
 */
export function useCompetitorCitationData(pageAnalyses: Ref<any[]>, competitors: Ref<any[]>, totalCitations: Ref<number>) {
  // Competitor citation data
  const competitorsWithCitationData = computed(() => {
    const competitorsList = competitors.value || []
    const pageAnalysesList = pageAnalyses.value || []
    
    if (pageAnalysesList.length === 0) {
      return []
    }
