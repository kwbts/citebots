-- Queue Infrastructure for Large-Scale Analysis Processing
...
CREATE TABLE IF NOT EXISTS analysis_queue (
  ...
  CONSTRAINT idx_queue_status_created UNIQUE (status, created_at, id)
);
...
ADD COLUMN IF NOT EXISTS processing_method TEXT DEFAULT 'sync' CHECK (processing_method IN ('sync', 'queue'));
...
-- 7. Add RLS policies for queue table
