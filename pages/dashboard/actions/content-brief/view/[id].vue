<template>
  <div class="max-w-7xl mx-auto">
    ...
    <div v-if="debugInfo.error || isLoading" class="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
      <div class="flex items-center justify-between mb-2">
        <h3 class="text-red-800 dark:text-red-400 font-semibold">Brief Loading Status</h3>
        <span class="px-2 py-1 text-xs rounded-full" 
          :class="{
            'bg-green-100 dark:bg-green-800/30 text-green-800 dark:text-green-400': !isLoading && !debugInfo.error,
            'bg-yellow-100 dark:bg-yellow-800/30 text-yellow-800 dark:text-yellow-400': isLoading,
            'bg-red-100 dark:bg-red-800/30 text-red-800 dark:text-red-400': !isLoading && debugInfo.error
          }">
          {{ isLoading ? 'Loading...' : (debugInfo.error ? 'Error' : 'Loaded') }}
        </span>
      </div>
      
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div>
          <p class="text-sm font-medium text-gray-700 dark:text-gray-300">Supabase</p>
          <div class="flex items-center mt-1">
            <span class="h-2 w-2 rounded-full mr-2" 
              :class="{
                'bg-green-500': debugInfo.supabaseStatus === 'connected',
                'bg-red-500': debugInfo.supabaseStatus === 'error',
                'bg-yellow-500 animate-pulse': debugInfo.supabaseStatus === 'checking'
              }"></span>
      ...
      <div v-if="debugInfo.error" class="p-3 bg-red-100 dark:bg-red-900/30 rounded text-sm text-red-800 dark:text-red-300 mb-3">
        {{ debugInfo.error }}
      </div>
      
      <div class="text-sm text-gray-600 dark:text-gray-400">
        <p><strong>Brief ID:</strong> {{ debugInfo.briefId }}</p>
        <p v-if="debugInfo.briefDataSource"><strong>Data Source:</strong> {{ debugInfo.briefDataSource }}</p>
      </div>
    </div>
          ...
          <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            <div class="mb-4 md:mb-0">
              ...
              <div class="mt-1 flex flex-wrap items-center text-sm text-gray-500 dark:text-gray-400">
                <span>{{ brief.client?.name || 'Client' }}</span>
              ...
              <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <div class="text-xs text-gray-600 dark:text-gray-400 space-y-2">
                  <div class="flex items-center">
                    ...
                    <span>{{ brief.meta?.research_stats?.llm_queries_executed || 3 }} LLM queries</span>
                  </div>
                  <div class="flex items-center">
                    <svg class="w-4 h-4 text-green-500 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span>{{ brief.meta?.research_stats?.pages_analyzed || 7 }} pages analyzed</span>
                  </div>
                  <div class="flex items-center">
                    <svg class="w-4 h-4 text-green-500 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span>{{ brief.meta?.research_stats?.competitor_pages_analyzed || 3 }} competitor pages</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700/60 p-6">
            ...
            <div v-if="activeTab === 'overview'" class="space-y-6">
              ...
              <div class="prose dark:prose-invert max-w-none">
                <p class="whitespace-pre-line">{{ brief.strategic_overview || summaryPlaceholder }}</p>
              </div>

              <!-- Brand Voice Guidelines -->
              <div v-if="brief.brand_voice_guidelines" class="mt-8">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Brand Voice Guidelines</h3>
                <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                  <p class="text-gray-900 dark:text-white whitespace-pre-line">{{ brief.brand_voice_guidelines }}</p>
                </div>
              </div>

              <!-- Specificity Requirements -->
              <div v-if="brief.specificity_requirements" class="mt-8">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Specificity Requirements</h3>
                <div class="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800">
                  <p class="text-gray-900 dark:text-white whitespace-pre-line">{{ brief.specificity_requirements }}</p>
                </div>
              </div>

              <!-- Content Differentiation -->
              <div v-if="brief.content_differentiation" class="mt-8">
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">Content Differentiation Strategy</h3>
                <div class="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-200 dark:border-orange-800">
                  <p class="text-gray-900 dark:text-white whitespace-pre-line">{{ brief.content_differentiation }}</p>
                </div>
              </div>
            </div>
            ...
            <div v-else-if="activeTab === 'toc'" class="space-y-6">
              ...
              <div class="space-y-8">
                <div 
                  v-for="(section, idx) in brief.table_of_contents || placeholderTOC" 
                  :key="idx"
            ...
            <div v-else-if="activeTab === 'statistics'" class="space-y-6">
              ...
              <div v-if="brief.quotable_statistics?.length" class="space-y-4">
                <div 
                  v-for="(stat, idx) in brief.quotable_statistics" 
                  :key="idx"
            ...
            <div v-else-if="activeTab === 'research'" class="space-y-6">
              ...
              <div class="space-y-4">
                <div 
                  v-for="(source, idx) in brief.research_links || placeholderResearch" 
                  :key="idx"
                  class="bg-gray-50 dark:bg-gray-700/40 p-4 rounded-lg border border-gray-200 dark:border-gray-700"
                >
                  <div class="flex items-start">
            ...
            <div v-else-if="activeTab === 'competitor'" class="space-y-6">
              ...
              <div v-if="brief.competitive_landscape_analysis" class="space-y-6">
                <div class="bg-gray-50 dark:bg-gray-700/40 p-6 rounded-lg border border-gray-200 dark:border-gray-700">
                  <div class="prose dark:prose-invert max-w-none">
                    <div class="whitespace-pre-line text-gray-900 dark:text-white">{{ brief.competitive_landscape_analysis }}</div>
                  </div>
                </div>
              </div>
              
              <!-- Fallback to legacy competitor insights if new analysis not available -->
              <div v-else-if="brief.process_notes?.competitor_insights?.length" class="space-y-4">
                <div 
                  v-for="(insight, idx) in brief.process_notes.competitor_insights" 
                  :key="idx"
                  class="bg-gray-50 dark:bg-gray-700/40 p-4 rounded-lg border border-gray-200 dark:border-gray-700"
                >
            ...
            <div v-else-if="activeTab === 'process'" class="space-y-6">
              <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Process Notes</h2>
              ...
              <div class="space-y-6">
                <div>
                  ...
                  <div class="space-y-3">
                    <div 
                      v-for="(query, idx) in brief.process_notes?.llm_responses || placeholderQueries" 
                      :key="idx"
                ...
                <div>
                  ...
                  <div class="space-y-3">
                    <div 
                      v-for="(result, idx) in brief.process_notes?.search_results || placeholderSearchResults" 
                      :key="idx"
                      class="bg-gray-50 dark:bg-gray-700/40 p-3 rounded-lg border border-gray-200 dark:border-gray-700"
                    >
...
const debugInfo = ref({
  ...
  error: null,
  briefDataSource: null,
  briefId: route.params.id
})

// Check connectivity to Supabase
async function checkConnectivity() {
  debugInfo.value.loading = true
  
  // Check Supabase
  try {
    const { data, error } = await supabase.from('content_briefs').select('count').limit(1)
    if (error) {
      throw error
    }
    debugInfo.value.supabaseStatus = 'connected'
  } catch (error) {
    console.error('Supabase connectivity error:', error)
    debugInfo.value.supabaseStatus = 'error'
  }
  
  debugInfo.value.loading = false
}
...
const placeholderTOC = [
  ...
  {
    title: "4. Content Strategy Optimized for SaaS Conversion",
    points: [
      "Creating high-converting comparison and alternatives pages",
      "Developing SEO-optimized product documentation that ranks",
      "Thought leadership content that builds authority in your niche",
      "Content upgrade strategies to capture leads from organic traffic"
    ]
  },
  {
    title: "5. Measuring SEO Success for SaaS Businesses",
    points: [
      "Key performance indicators beyond rankings and traffic",
      "Integrating SEO analytics with product usage and retention metrics",
      "Attribution modeling for organic traffic throughout the SaaS funnel",
      "Calculating true customer acquisition cost from organic search"
    ]
  }
]
...
const placeholderResearch = [
  ...
  {
    title: "SaaS Customer Journey Mapping for Content Strategy",
    url: "https://example.com/saas-journey-mapping",
    description: "Framework for aligning SEO content with specific stages of the SaaS customer journey",
    source_type: "research"
  }
]
...
4. Case Studies: Competitor content relies heavily on theory and best practices but lacks concrete case studies with measurable results from actual SaaS implementations.

5. Actionability: Most content is informational rather than providing step-by-step frameworks that readers can immediately implement.`
...
const fetchBrief = async () => {
  ...
  try {
    ...
    brief.value = {
      ...
      client: briefData.client_id ? {
        id: briefData.client_id,
        name: 'Client', // We don't have client name in this data
        domain: ''
      } : undefined,
      keywords: briefData.keywords || [],
      purpose: briefData.purpose || '',
      audience: briefData.audience || '',
      generated_at: briefData.created_at,
      summary: briefData.content?.summary || '', // Legacy field
      strategic_overview: briefData.content?.strategic_overview || '',
      brand_voice_guidelines: briefData.content?.brand_voice_guidelines || '',
      specificity_requirements: briefData.content?.specificity_requirements || '',
      content_differentiation: briefData.content?.content_differentiation || '',
      quotable_statistics: briefData.content?.quotable_statistics || [],
      competitive_landscape_analysis: briefData.content?.competitive_landscape_analysis || '',
      content_differentiation: briefData.content?.content_differentiation || '',
      content_suggestions: briefData.content?.content_suggestions || [], // Legacy field
      table_of_contents: briefData.content?.table_of_contents || [],
      research_links: briefData.content?.research_links || [],
      process_notes: briefData.content?.process_notes || {
        llm_responses: [],
        search_results: [],
        competitor_insights: []
      },
      meta: briefData.content?.meta || {
        research_stats: {
    ...
    debugInfo.value.briefDataSource = 'supabase'
  } catch (err) {
    console.error('Error fetching brief:', err)
    error.value = err.message || 'Error fetching brief data'
    debugInfo.value.error = err.message || 'Unknown error fetching brief'
    debugInfo.value.briefDataSource = 'error'
  } finally {
...
const shareBrief = () => {
  ...
  const shareUrl = `${window.location.origin}/dashboard/actions/content-brief/view/${route.params.id}`;
...
const copyStatistic = (stat) => {
  ...
  navigator.clipboard.writeText(text)
    ...
    .catch(err => {
      console.error('Failed to copy statistic:', err);
      alert('Failed to copy statistic');
    });
}
...
function formatBriefAsText(brief) {
  ...
  text += `## Strategic Overview\n\n${brief.strategic_overview || brief.summary || 'No strategic overview available'}\n\n`
  ...
  if (brief.content_differentiation) {
    text += `## Content Differentiation Strategy\n\n${brief.content_differentiation}\n\n`
  }
  
  if (brief.competitive_landscape_analysis) {
    text += `## Competitive Landscape Analysis\n\n${brief.competitive_landscape_analysis}\n\n`
  }
...
function formatBriefAsHtml(brief) {
  let html = `
    ...
    <body>
      ...
      <p>${brief.strategic_overview || brief.summary || 'No strategic overview available'}</p>
