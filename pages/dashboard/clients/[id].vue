<template>
  <div class="max-w-4xl mx-auto">
    ...
    <div v-if="isLoading" class="text-center py-12">
      ...
      <p class="mt-4 text-citebots-gray-600">Loading client...</p>
    </div>
    ...
    <div v-else-if="client">
      ...
      <div class="card mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p class="text-citebots-dark">{{ client.name || 'Not specified' }}</p>
          </div>
          <div>
            <p class="form-label">Domain</p>
            <p class="text-citebots-dark">{{ client.domain || 'Not specified' }}</p>
          </div>
          <div>
            <p class="form-label">Created</p>
            <p class="text-citebots-dark">{{ new Date(client.created_at).toLocaleDateString() }}</p>
          </div>
          <div>
            <p class="form-label">Last Updated</p>
            <p class="text-citebots-dark">{{ new Date(client.updated_at).toLocaleDateString() }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="card mb-6">
        <h2 class="text-xl font-semibold mb-4">Industry Information</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <p class="form-label">Primary Industry</p>
            <p class="text-citebots-dark">{{ client.industry_primary || 'Not specified' }}</p>
          </div>
          <div>
            <p class="form-label">Secondary Industry</p>
            <p class="text-citebots-dark">{{ client.industry_secondary || 'Not specified' }}</p>
          </div>
          <div>
            <p class="form-label">Business Model</p>
            <p class="text-citebots-dark">{{ client.business_model || 'Not specified' }}</p>
          </div>
          <div>
            <p class="form-label">Sub-Industry</p>
            <p class="text-citebots-dark">{{ client.sub_industry || 'Not specified' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="card mb-6">
        <h2 class="text-xl font-semibold mb-4">Target Market & Products</h2>
        <div class="space-y-4">
          <div>
            ...
            <div v-if="client.target_audience?.length" class="flex flex-wrap gap-2">
              <span v-for="(item, index) in client.target_audience" :key="index"
                    class="inline-block bg-citebots-gray-100 text-citebots-gray-800 px-3 py-1 rounded-full text-sm">
                {{ item }}
              </span>
            </div>
            <p v-else class="text-citebots-gray-500">Not specified</p>
          </div>
      ...
      <!-- Business Context -->
      <div class="card mb-6">
        <h2 class="text-xl font-semibold mb-4">Business Context</h2>
        <div class="space-y-4">
          <div>
      ...
      <div class="card mb-6">
        ...
        <div v-if="client.competitors?.length > 0" class="space-y-3">
          <div v-for="competitor in client.competitors" :key="competitor.id" 
               class="border border-citebots-gray-200 rounded-lg p-4 hover:border-citebots-gray-300 transition-colors">
            <div class="flex justify-between items-start">
              <div>
                <h3 class="font-medium text-citebots-dark">{{ competitor.name }}</h3>
                <p class="text-citebots-gray-600">{{ competitor.domain }}</p>
              </div>
        ...
        <div v-else class="text-center py-8 text-citebots-gray-500">
      ...
      <div class="card mb-6">
        ...
        <p v-else class="text-citebots-gray-500">No keywords defined</p>
      </div>
      ...
      <div v-if="client.ai_enhancement_count > 0" class="card mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p class="text-citebots-dark">{{ client.ai_enhancement_count }} times</p>
          </div>
          <div>
            <p class="form-label">Last Enhanced</p>
            <p class="text-citebots-dark">{{ client.ai_enhanced_at ? new Date(client.ai_enhanced_at).toLocaleDateString() : 'N/A' }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-semibold text-citebots-dark mb-4">Confirm Delete</h3>
        <p class="text-citebots-gray-600 mb-6">
